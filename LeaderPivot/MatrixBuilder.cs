    ...
    public class MatrixBuilder<T>
    {
        ...
        private void BuildRows(Node<T> node, Matrix t, int index, int peerDepth)
        {
            ...
            if (node.IsRow)
            {
                ...
                {
                    rowSpan = Math.Max(GetLeafNodeCount(node, true, 0),1);
                }
        ...
        private int GetHeaderDepth(Node<T> node, bool checkRows, int maxDepth)
        {
            ...
            foreach (Node<T> child in node.Children.Where(x => (x.IsRow && checkRows || !x.IsRow && !checkRows) && x.IsExpanded))
        ...
        private int GetLeafNodeCount(Node<T> node, bool checkRows, int count)
        {
            int tmp = 0;
            ...
            if ((checkRows && node.IsRow) || (!checkRows && !node.IsRow))
                if (!node.IsExpanded || !node.Children.Any(x => (checkRows && x.IsRow) || (!checkRows && !x.IsRow)))
                    count = 1;
